...
void LtlFormulaTriple::simplifyNext() {
    // combine step and left clauses that share the same RHS
    unordered_map<Literal, vector<literal_set>, LiteralHash, LiteralEqual> stepClausesByRhs;
    for (LtlClause stepClause : stepClauses) {
    ...
    for (auto& entry : stepClausesByRhs) {
        ...
        if (entry.second.size() == 1) {
            ...
            Literal newLit = Literal("$x"+rhs.toString(), true);
...
void LtlFormulaTriple::simplifyEventuality() {
    ...
    unordered_map<Literal, vector<literal_set>, LiteralHash, LiteralEqual> eventualitiesByRhs;
    ...
    for (auto& entry : eventualitiesByRhs) {
        ...
        if (entry.second.size() == 1) {
            ...
            Literal newLit = Literal("$d"+rhs.toString(), true);
            for (literal_set clause : entry.second) {
                literal_set newClause {newLit};
                for (auto x : clause) newClause.insert(~x);
                clauses.push_back(newClause);
...
So use: [] nnv8 -> X nnv5
    ...
    for (literal_set clause : clauses) {
        ...
        if (neg == 1) {
            literal_set implied;
            for (Literal lit : clause) if (lit != lhs) implied.insert(lit);
            imps[lhs].push_back(litsetString(implied));
        } else {
            // We are not including this clause, so avoid literals that appear here
    ...
    literal_set remove;
    unordered_map<Literal, Literal, LiteralHash, LiteralEqual> replaceEventualities;
    for (auto x : nextOrs) {
        ...
        if (impliedBy.find(key) != impliedBy.end()) {
            cout << "Adding step: " << x.first.toString() << " -> " << (~impliedBy.at(key)).toString() << endl;
    ...
    for (LtlClause clause : stepClauses) {
        if (remove.find(clause.right) == remove.end()) newStepClauses.push_back(clause);
    }
    ...
    for (LtlClause clause : eventualityClauses) {
        //Literal eventRhs =  Literal(clause.right.getName().substr(2), clause.right.getPolarity());
        ...
        if (stepRhs.find(eventRhs) != stepRhs.end()) {
            ...
            newClause = {
                ...
                Literal(clause.right.getName(), clause.right.getPolarity()), fresh};
    ...
    for (LtlClause clause : eventualityClauses) {
        ...
        if (replaceEventualities.find(clause.right) != replaceEventualities.end()) {
            Literal fresh = Literal("$d"+to_string(count++), true);
            addClause({fresh, ~replaceEventualities.at(clause.right)});
            addClause({fresh, ~(*clause.left.begin())});
            newEventualityClauses.push_back(LtlClause({fresh}, replaceEventualities.at(clause.right)));
        } else {
